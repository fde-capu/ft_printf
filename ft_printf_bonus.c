...
/*   Updated: 2020/03/12 19:25:03 by fde-capu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ftpf_render(va_list ap, t_ttable *t)
{
	...
	t->s = t->t == 'd' && !t->l ? \
		ft_xlloc(t->s, ft_itoa(va_arg(ap, int))) : t->s;
	t->s = t->t == 'd' && t->l == 1 ? \
		ft_xlloc(t->s, ft_ltoa(va_arg(ap, long))) : t->s;
	t->s = t->t == 'd' && t->l == 2 ? \
		ft_xlloc(t->s, ft_lltoa(va_arg(ap, long))) : t->s;
	t->s = t->t == 'c' ? \
